...
function LabBookingForm({ tests = [] }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!selectedTest) { setLocalErrors({ test: t('booking.local_error') }); return; }
    setLocalErrors({});
    submit();
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4" noValidate>
      <h3 className="text-lg font-bold text-slate-900 mb-5" style={{ fontFamily: 'Outfit, sans-serif' }}>{t('booking.test_book')}</h3>
      <div>
        <label className="text-sm font-medium text-slate-700 block mb-1.5">{t('booking.select_test_label')}<span className="text-red-500">*</span></label>
        <select
          value={selectedTest}
        ...
        >
          <option value="">{t('booking.select_test')}</option>
          {tests.map(test => (
            <option key={test.id} value={test.name}>{t(`testNames.${test.name}`)} — {test.price} MAD</option>
          ))}
        </select>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Input label={t('booking.name')} name="name" value={form.name} onChange={handleChange} error={errors.name} placeholder="Your name" required icon={FiUser} />
        <Input label={t('booking.phone')} name="phone" value={form.phone} onChange={handleChange} error={errors.phone} placeholder="+212 6XX" required icon={FiPhoneIcon} />
      </div>
      <Input label={t('booking.email')} name="email" type="email" value={form.email} onChange={handleChange} error={errors.email} placeholder="email@example.com" required icon={FiMailIcon} />
      <Input label={t('booking.date')} name="date" type="date" value={form.date} onChange={handleChange} error={errors.date} required icon={FiCalendar} min={today} />
      ...
      <Button type="submit" className="w-full" size="lg" loading={loading}>{t('booking.submit')}</Button>
    </form>
  );
}
  ...
  if (!lab) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-4">
      ...
      <Link to="/labs" className="text-teal-700 flex items-center gap-2 hover:underline">
        <FiArrowLeft size={16} /> {t('labs.back')}
      </Link>
    </div>
  );
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        <div className="flex gap-1 bg-white border border-slate-100 rounded-xl p-1 mb-8 w-fit">
          {[{key: 'tests', label: t('tabs.tests')},
            {key: 'about', label: t('tabs.about')},
            {key: 'booking', label: t('tabs.booking')}
          ].map(tab => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={`px-5 py-2 rounded-lg text-sm font-medium capitalize transition-colors ${activeTab === tab.key ? 'bg-teal-700 text-white' : 'text-slate-600 hover:bg-slate-50'}`}
            >{tab.label}</button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div key={test.id} className="bg-white border border-slate-100 rounded-2xl p-5 flex items-center justify-between gap-4 card-hover">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <Badge variant="blue">{t(`testCategories.${test.category}`)}</Badge>
                        </div>
                        <p className="font-semibold text-slate-900 text-sm">{t(`testNames.${test.name}`)}</p>
                        <p className="text-xs text-slate-400 mt-1">{t('Turnaround')}: {test.turnaround}</p>
                      </div>
            ...
            {activeTab === 'about' && (
              <div className="space-y-6">
                <div className="bg-white rounded-2xl border border-slate-100 p-6">
                  <h2 className="font-bold text-slate-900 text-lg mb-3">{t('tabs.about')}</h2>
                  <p className="text-slate-600 text-sm leading-relaxed">{lab.description}</p>
                </div>
                <div className="bg-white rounded-2xl border border-slate-100 p-6">
                  <h2 className="font-bold text-slate-900 text-lg mb-4">{t('tabs.Certifications')}</h2>
                ...
                <div className="bg-white rounded-2xl border border-slate-100 p-6">
                  <h2 className="font-bold text-slate-900 text-lg mb-4">{t('tabs.services')}</h2>
                  <div className="flex flex-wrap gap-2">{lab.services.map(s => <Badge key={s} variant="teal">{s}</Badge>)}</div>
                </div>
                <div className="bg-white rounded-2xl border border-slate-100 p-6">
                  <h2 className="font-bold text-slate-900 text-lg mb-4">{t('tabs.Amenities')}</h2>
          ...
          <div className="space-y-5">
            <div className="bg-white rounded-2xl border border-slate-100 p-5">
              <h3 className="font-bold text-slate-900 text-base mb-4">{t('contact.info')}</h3>
            ...
            <div className="bg-teal-50 border border-teal-100 rounded-2xl p-5 text-center">
              <p className="text-sm font-semibold text-teal-800 mb-1">{t('contact.need_labo')}</p>
              <p className="text-xs text-teal-600 mb-3">{t('contact.come_labo')}</p>
